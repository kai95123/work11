<?php
	...
	/**  
	 * Copyright 2017  Juan Sanchez Jr. ( email : bringmesupport@gmail.com )
	...
	class jsjr_personal_contact_info extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if ( !empty( $email ) && !empty( $email_size ) && !empty( $email_spacing ) && !empty( $email_clickable )){
                            if ( $email_clickable  === 'checked' ) {   
                                echo '<p class="jsjr-pci-email" style="font-size:', $email_size , '; margin-bottom:' , $email_spacing , '"><a href="mailto:' , $email , '">', $email, '</a></p>';
                            } elseif ( $email_clickable  === 'unchecked' ) {
                                echo '<p class="jsjr-pci-email" style="font-size:', $email_size , '; margin-bottom:' , $email_spacing , '">' , $email , '</p>';
                            }
			}
			
			if ( !empty( $phone ) && !empty( $phone_size ) && !empty( $phone_spacing ) && !empty( $phone_clickable ) ) {
                            if ( $phone_clickable  === 'checked' ) {   
                                echo '<p class="jsjr-pci-phone" style="font-size:', $phone_size , '; margin-bottom:' , $phone_spacing , '"><a href="tel:' , $phone , '">', $phone , '</a></p>';
                            } elseif ( $phone_clickable  === 'unchecked' ) {
                                echo '<p class="jsjr-pci-phone" style="font-size:', $phone_size , '; margin-bottom:' , $phone_spacing , '">' , $phone , '</p>';
                            }
                        }
			
			if ( !empty( $website ) && !empty( $website_size ) && !empty( $website_spacing ) && !empty( $website_clickable ) ) {
                            if ( $website_clickable  === 'checked' ) {                               
                                if (0 === strpos($string2, 'http')) {
                                    echo '<p class="jsjr-pci-website" style="font-size:', $website_size , '; margin-bottom:' , $website_spacing , '"><a href="' , $website , '" target="_blank">' , $website, '</a></p>';
                                } else {
                                    echo '<p class="jsjr-pci-website" style="font-size:', $website_size , '; margin-bottom:' , $website_spacing , '"><a href="http://' , $website , '" target="_blank">' , $website, '</a></p>';
                                }
                            } elseif( $website_clickable  === 'unchecked' ) {
		...
		public function update( $new_instance, $old_instance ) {
			...
			$old_instance[ 'fa_existing' ] = isset( $new_instance[ 'fa_existing' ] ) ? $new_instance[ 'fa_existing' ] : 'unchecked';
                        $old_instance[ 'email_clickable' ] = isset( $new_instance[ 'email_clickable' ] ) ? $new_instance[ 'email_clickable' ] : 'unchecked';
                        $old_instance[ 'phone_clickable' ] = isset( $new_instance[ 'phone_clickable' ] ) ? $new_instance[ 'phone_clickable' ] : 'unchecked';
                        $old_instance[ 'website_clickable' ] = isset( $new_instance[ 'website_clickable' ] ) ? $new_instance[ 'website_clickable' ] : 'unchecked';
                        return $old_instance;
		}
		...
		public function form( $instance ) {	
                    ...
                    $defaults = array (
                        ...
                        'email_size' 			=> '15px',
                        'email_spacing'			=> '20px',
                        'email_clickable'               => 'unchecked',
                        'phone'				=> '',
                        'phone_size' 			=> '15px',
                        'phone_spacing'			=> '20px',
                        'phone_clickable'               => 'unchecked',
                        'website'			=> '',
                        'website_size'			=> '15px',				
                        'website_spacing'		=> '20px',
                        'website_clickable'		=> 'unchecked',
                    ...
                    <div class="jsjr-pci-accordion">
                        ...
                        <div class="contact-section" style="display:none;" >
                            ...
                            <hr/>
                            <?php $this->get_contact_section( 'Full Name:', 'full_name', $full_name, $full_name_size, $full_name_spacing, FALSE ); ?>

                            <hr />					
                            <?php $this->get_contact_section( 'Slogan:', 'slogan', $slogan, $slogan_size, $slogan_spacing, FALSE ); ?>

                            <hr />					
                            <?php $this->get_contact_section( 'Email:', 'email', $email, $email_size, $email_spacing, $email_clickable ); ?>

                            <hr />					
                            <?php $this->get_contact_section( 'Phone:', 'phone', $phone, $phone_size, $phone_spacing, $phone_clickable ); ?>

                            <hr />					
                            <?php $this->get_contact_section( 'Alternate Website:', 'website', $website, $website_size, $website_spacing, $website_clickable ); ?>
                            <hr />
		 ...
		 */	
		public function get_contact_section( $description, $field_name, $field_name_text , $field_name_size, $field_name_spacing, $field_name_clickable ) {
                    ...
                    if ($field_name_clickable !== FALSE){
                        $this->get_field_checkbox( $args = array(
                            'field_name'        => $field_name.'_clickable',
                            'instance_value'    => $field_name_clickable,
                            'description'       => 'Make this clickable',
                            'return_value'      => 'checked'
                        ));
                    }
		}
